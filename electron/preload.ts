...
const electronAPI = {
  ...
  speech: {
    // 获取语音识别配置
    getConfig: () => ipcRenderer.invoke('speech:getConfig') as Promise<{
    ...
    setConfig: (config: {
      ...
      whisperApiKey?: string
      funasrWsUrl?: string
    }) => ipcRenderer.invoke('speech:setConfig', config),

    // 检查本地模型是否可用
    checkLocalModel: () => ipcRenderer.invoke('speech:checkLocalModel') as Promise<boolean>,

    // 获取当前状态
    getState: () => ipcRenderer.invoke('speech:getState') as Promise<'idle' | 'recording' | 'processing'>,

    // 开始录音
    startRecording: () => ipcRenderer.invoke('speech:startRecording') as Promise<boolean>,

    // 停止录音
    stopRecording: () => ipcRenderer.invoke('speech:stopRecording') as Promise<{
      text: string
      isFinal: boolean
      confidence?: number
    } | null>,
    ...
    sendAudioData: (audioData: number[]) => {
      ipcRenderer.send('speech:audioData', audioData)
    },

    // 监听部分识别结果
    onPartialResult: (callback: (result: { text: string; isFinal: boolean; confidence?: number }) => void) => {
      const handler = (_event: Electron.IpcRendererEvent, result: { text: string; isFinal: boolean; confidence?: number }) => callback(result)
      ipcRenderer.on('speech:partialResult', handler)
      return () => {
        ipcRenderer.removeListener('speech:partialResult', handler)
      }
    },

    // 监听状态变化
    onStateChange: (callback: (state: 'idle' | 'recording' | 'processing') => void) => {
      const handler = (_event: Electron.IpcRendererEvent, state: 'idle' | 'recording' | 'processing') => callback(state)
      ipcRenderer.on('speech:stateChange', handler)
      return () => {
        ipcRenderer.removeListener('speech:stateChange', handler)
      }
    },
    ...
    onError: (callback: (error: string) => void) => {
      ...
      return () => {
        ipcRenderer.removeListener('speech:error', handler)
      }
    }
  }
}

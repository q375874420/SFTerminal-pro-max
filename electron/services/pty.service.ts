...
export class PtyService {
  ...
  /**
   * 发送命令到终端并等待用户按回车执行（严格模式用）
   * 命令先显示在终端输入行，用户可以查看/修改后按回车执行
   */
  sendCommandAndWaitForExecution(
    id: string,
    command: string,
    timeout: number = 60000,  // 等待用户操作的超时时间，默认 60 秒
    silenceTimeout: number = 2000
  ): Promise<{ output: string; duration: number; cancelled: boolean }> {
    return new Promise((resolve) => {
      const instance = this.instances.get(id)
      if (!instance) {
        resolve({ output: '终端实例不存在', duration: 0, cancelled: false })
        return
      }
      ...
      let timeoutTimer: NodeJS.Timeout | null = null
      let resolved = false
      let executionStarted = false  // 标记用户是否开始执行（按了回车）
      ...
      const finish = (cancelled: boolean = false) => {
        if (resolved) return
        resolved = true
        cleanup()
        resolve({
      ...
      const resetSilenceTimer = () => {
        if (silenceTimer) clearTimeout(silenceTimer)
        silenceTimer = setTimeout(() => finish(false), silenceTimeout)
      }

      // 输出处理器
      const outputHandler = (data: string) => {
        // 检测用户按了回车（收到换行符后的输出表示命令开始执行）
        if (!executionStarted) {
          // 检测换行符，表示用户按了回车
          if (data.includes('\n') || data.includes('\r')) {
          ...
          // 检测 Ctrl+C（用户取消）
          if (data.includes('\x03') || data.includes('^C')) {
            finish(true)
            return
          }
        }
